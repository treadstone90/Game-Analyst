...
trait ComboFilterable {
  def getQuery(ids:Array[String],terms:Array[String]) :FilterQuery
}
...
trait TermFollowFilter extends ComboFilterable {
  def getQuery(ids:Array[String],track:Array[String]) = 
  QueryBuilder.trackFollow(ids,track);
}
...
object QueryBuilder {
  ...
  def trackFollow(idStrings :Array[String] ,terms:Array[String]) = 
  new FilterQuery(10,idStrings.map(_.toLong),terms);
